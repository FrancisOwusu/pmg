...
<div class="template-container printable" style="width: {{ $print->width }}; height: {{ $print->height }};">
  <div class="template-inner">
    ...
    @php
        $enroll = \App\Models\Student::enroll($row->fee->studentEnroll->student->id);
    @endphp
    <!-- Header Section -->
    ...
    <table class="table-no-border receipt">
        ...
        <tbody>
            ...
            <tr class="border-bottom">
                ...
                <td>-
                    @if(isset($setting->decimal_place))
                        {{ number_format((float)$row->self_payments, $setting->decimal_place, '.', '') }}
                    @else
                        {{ number_format((float)$row->self_payments, 2, '.', '') }}
                    @endif
                    {!! $setting->currency_symbol !!}
                </td>
            </tr>
            <tr class="tfoot">
                ...
                <th>=
                    @if(isset($setting->decimal_place))
                    {{ number_format((float)$row->paid_amount + $row->self_payments, $setting->decimal_place, '.', '') }}
                    @else
                    {{ number_format((float)$row->paid_amount + $row->self_payments, 2, '.', '') }}
                    @endif
                    {!! $setting->currency_symbol !!}
                </th>
            </tr>
