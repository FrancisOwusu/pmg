                            ...
                            <aside class="sidebar-widget info-column">
                                <div class="inner-column3">
                                    ...
                                    <ul class="project-info clearfix">
                                        ...
                                        @if(!empty($course->fee))
                                        <li>
                                            <strong>{{ __('field_total') }} {{ __('field_fee') }} ({{__('per_semester')}}): </strong>
                                            <span>{!! $setting->currency_symbol !!} {{ round($course->fee, $setting->decimal_place ?? 2) }} </span>
                                        </li>
                                        @endif
                                            @if(!empty($course->hostel_fee))
                                                <li>
                                                    <strong>{{ __('field_hostel_fee') }} ({{__('per_semester')}}): </strong>
                                                    <span>{!! $setting->currency_symbol !!} {{ round($course->hostel_fee, $setting->decimal_place ?? 2) }} </span>
                                                </li>
                                            @endif
                                    </ul>
                                </div>
