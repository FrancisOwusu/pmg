...
Route::get('testm', function () {
    $query = DB::table('students')
        ...
        ->first();
// If database is empty, start from 1001
    $next_id = ($query->last_id !==NULL)? $query->last_id + 1 : 1;
    $student_id = $next_id + 1000; // Add 1000 when displaying
...
Route::post("payments/callback", [\App\Http\Controllers\Student\FeePaymentController::class, 'paymentResponse'])->name('student_fees.callback');
Route::post("payments/fees", [\App\Http\Controllers\Student\FeePaymentController::class, 'processPayment'])->name('student_fees.pay');
//Route::post("payments/{reference}", [\App\Http\Controllers\Admin\PaymentController::class, 'processPayment'])->name('student_fees.pay');
...
Route::middleware(['auth:web', 'XSS'])->name('admin.')->namespace('Admin')->prefix('admin')->group(function () {
    ...
    Route::prefix('payments')->group(function (){
        ...
        Route::get('/pint/{id}',[\App\Http\Controllers\Admin\PaymentController::class,'print'])->name('payments.print');
    });
    // Student Routes
